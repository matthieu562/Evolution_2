...
class Cell:
    ...
    def create_new_cells(cls, space, nb_new_cell, images, x=None, y=None, angle=0, radius=None):
        ...
        for i in range(nb_new_cell):
            if x == None: x=random.randint(0, WINDOW_WIDTH)
            if y == None: y=random.randint(0, WINDOW_HEIGHT)
            if radius == None: radius=random.randint(CELL_MIN_SIZE, CELL_MAX_SIZE)
            color=(122, 0, 122) if i != 0 else (0, 45, 223)
            # image=image
            new_cells.append(Cell(space, (x, y), radius, color, images, angle))
        return new_cells
    ...
    def losses_life_points(self, oppressor):
        ...
        self.life_points -= oppressor.damage
    ...
    def get_birth(self):
        if self.energy > 50 and menu_globals.game_clock - self.last_reproduction_date > REPRODUCTION_DELAY*FPS:
            ...
            return Cell.create_new_cells(self.space, 1, self.images, x, y, self.body.angle + math.pi, self.shape)
